...
function treemap(slice, payload) {
    ...
    const _draw = function (data, eltWidth, eltHeight, formData) {
        ...
        const treemap = d3.layout.treemap()
            ...
            .ratio(formData.treemap_Opvizor_ratio)
            .mode('squarify')
            .round(false);
        ...
        const display = function (d) {
            ...
            g.selectAll('rect')
                .style('fill', function (d) {
                    ...
                    if(d.parent.name == 'error'){return errorspectrumcolors(d.name);}
                    else{
                        if(d.parent.name == 'warning'){return warningspectrumcolors(d.name);}
                        else {
                            if(d.parent.name == 'ok'){return okspectrumcolors(d.name);}
                        }
                    } });
